...
#define GPIO_OUTPUT_READER_POWER  12
#define GPIO_OUTPUT_RELAY_POWER   14
#define GPIO_OUTPUT_ACCESS_LED    32
#define GPIO_OUTPUT_CONNECTED_LED 21
#define GPIO_OUTPUT_PIN_SEL ((1ULL<<GPIO_OUTPUT_READER_POWER) | (1ULL<<GPIO_OUTPUT_RELAY_POWER) | (1ULL<<GPIO_OUTPUT_ACCESS_LED) | (1ULL<<GPIO_OUTPUT_CONNECTED_LED))
...
void bad_rfid_sequence () {
  ...
  blinkit_high (GPIO_OUTPUT_ACCESS_LED, 5, 250);
}
...
static void rfid_main_loop (void *pvParameters)
{
    ...
    while(1) {
        ...
        // see if data is coming from RFID reader
        //
        cardno_valid = get_RFID(&cardno);
        if (cardno_valid) {
            ESP_LOGI(TAG, "cardno = %lu", cardno);
        }

        if (cardno_valid && last_cardno_valid && (last_cardno == cardno)) {
            // don't bother looking anything up
            //
            attempts = MAX_TRIES;

        } else if (last_cardno_valid && (attempts > 0)) {
            // Different, bad, or no card:
            // don't turn off output or query server yet,
            // reset reader and try again
            //
            if (cardno_valid) {
                ESP_LOGI(TAG, "Different card sensed, %d tries left", attempts);
            } else {
                ESP_LOGI(TAG, "No card sensed, %d tries left", attempts);
            }
            attempts--;
            blinkit_low (GPIO_OUTPUT_ACCESS_LED, 10, 125);
            ...
            // if we used the cache, query the server to log the access
            if (used_cache) {
                query_rfid (cardno, true);
            }
        } else {
            // no card, or bad card and timed out

            // if we had a good card, log an invalid card number
       ...
       if (last_cardno_valid) {
           uart_flush_input(UART_NUM_1);
           // cut power on RFID reader, then restore
           gpio_set_level (GPIO_OUTPUT_READER_POWER, 0);
